...
//aRb8jJD5TRuFASrciUWMkjLOY9N2nw1oL9PsxZxsWcj
//Wgv4YnouVo6dj76H7RifTIvaVgOKUgSQcpl9mRqZSqz
...
const BitfinexAPIRoot = "https://www.bitmex.com/api/v1"
const NameBitfinex = "Bitfinex"

//对我们的 REST API 的请求是限于每 5 分钟 300 次的速率。此计数器持续重设。如果您没有登录，您的频率限制是每 5 分钟 150 次。

type ExchangeBitfinex struct {
	websocket *Websocket.Conn
	event     chan EventType
	config    Config
}

var BitfinexTradeTypeMap = map[TradeType]string{
	TradeTypeBuy:  "Buy",
	TradeTypeSell: "Sell",
}

func (p *ExchangeBitfinex) GetExchangeName() string {
	return NameBitfinex
}

// WatchEvent() return a channel which notified the application of the event triggered by exchange
func (p *ExchangeBitfinex) WatchEvent() chan EventType {
	return p.event
}

func (p *ExchangeBitfinex) Start() error {
	return nil
}

// Close() close the connection to the exchange and other handles
func (p *ExchangeBitfinex) Close() {

}

// StartTicker() send message to the exchange to start the ticker of the given pairs
func (p *ExchangeBitfinex) StartTicker(pair string) {
}

// CancelOrder() cancel the order as the order information
func (p *ExchangeBitfinex) CancelOrder(order OrderInfo) *TradeResult {
	return nil
}

// GetOrderInfo() get the information with order filter
func (p *ExchangeBitfinex) GetOrderInfo(filter OrderInfo) []OrderInfo {
	return nil
}

func (p *ExchangeBitfinex) GetKline(pair string, period int, limit int) []KlineValue {
	return nil
}

func (p *ExchangeBitfinex) GetTicker(pair string) *TickerValue {
	return nil
}

// SetConfigure()
func (p *ExchangeBitfinex) SetConfigure(config Config) {
	p.config = config
...
func (p *ExchangeBitfinex) orderRequest(method string, path string, params map[string]string) (error, []byte) {
...
func (p *ExchangeBitfinex) marketRequest(path string, params map[string]string) (error, []byte) {
	var req http.Request
	req.ParseForm()
	for k, v := range params {
		req.Form.Add(k, v)
...
func (p *ExchangeBitfinex) GetBalance() map[string]interface{} {
...
func (p *ExchangeBitfinex) Trade(configs TradeConfig) *TradeResult {
...
func (p *ExchangeBitfinex) GetComposite(symbol string, count int) (error, float64) {
...
func (p *ExchangeBitfinex) GetDepthValue(pair string) [][]DepthPrice {
	//ethusdt@depth20
...
func (p *ExchangeBitfinex) sign(method string, path string, expire string, data string) string {
...
var BitfinexOrderStatusMap = map[OrderStatusType]string{
	OrderStatusOpen:      "NEW",
...
func (p *ExchangeBitfinex) getStatusType(key string) OrderStatusType {

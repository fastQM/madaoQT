...
func (a *IAnalyzer) FixFailedPosition(updateJson string) error {
	...
	return errors.New(Task.TaskErrorMsg[Task.TaskInvalidInput])
		...
		for {
			...
			case event := <-futureExchange.WatchEvent():
				if event == Exchange.EventConnected {
					...
					if a.status != Task.StatusNone && a.status != Task.StatusError {
...
func (a *IAnalyzer) Watch() {
	...
	for coin := range a.config.Area {
		...
		err1, askFuture, askFuturePlacePrice, bidFuture, bidFuturePlacePrice := Task.CalcDepthPrice(true, constContractRatio, a.future, pair, a.config.UnitAmount)
		err2, askSpot, askSpotPlacePrice, bidSpot, bidSpotPlacePrice := Task.CalcDepthPrice(false, map[string]float64{}, a.spot, pair, a.config.UnitAmount)
...
func (a *IAnalyzer) placeOrdersByQuantity(future Exchange.IExchange, futureConfig Exchange.TradeConfig,
	...
	if futureResult.Error == Task.TaskErrorSuccess && spotResult.Error == Task.TaskErrorSuccess {
		...
		if spotResult.DealAmount > 0 {
			...
			go func() {
				...
				case spotResult = <-channelSpot:
					...
					if spotResult.Error != Task.TaskErrorSuccess {
		...
		waitGroup.Wait()
		if spotResult.Error == Task.TaskErrorSuccess && futureResult.Error == Task.TaskErrorSuccess {
	...
	if len(a.ops) != 0 {
		for index, op := range a.ops {
			...
			closeConditions := []bool{
				...
				Task.CheckPriceDiff(op.spotConfig, op.futureConfig, askFuturePrice, bidFuturePrice, askSpotPrice, bidSpotPrice, a.config.Area[coin].Close),
			}
